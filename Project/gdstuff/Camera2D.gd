...
func _process(_delta):
	if Main.camlock == false:
		if get_global_mouse_position().x > position.x && get_global_mouse_position().x < position.x + 16:
			position.x -= 8
		if get_global_mouse_position().y > position.y && get_global_mouse_position().y < position.y + 16:
			position.y -= 8
